...
def print_chart(participants_datasets, overall_results, cancer_type):
    tools = []
    x_values = []
    y_values = []
    for tool, metrics in participants_datasets.iteritems():
        tools.append(tool)
        x_values.append(overall_results[tool][1])
    ...
    ax.set_xlabel("Overall precision across all cancers", fontsize=12)
    ax.set_ylabel("Precision per cancer type", fontsize=12)
...
for participant, predicted_genes in all_cancer_genes.iteritems():
    ...
    overall_results[participant] = [TPR, acc]
...
#### plot chart with ALL cancer precision VS precision per cancer
...
for cancer in cancer_types:
    ...
    participants_datasets, all_cancer_genes = compute_metrics(input_dir, gold_standard, cancer,all_cancer_genes)
    tools_quartiles_squares, tools_quartiles_diagonal, tools_clusters = print_chart(participants_datasets, overall_results, cancer)
